...
export default function Dashboard({ initialCustomers, initialStats, initialActivities, userEmail, shopName = "Mahalla do'koni", liveMode, canManageMembers = false, initialError = "", initialView = "dashboard", initialCustomerId }: { initialCustomers: DashboardCustomer[]; initialStats: DashboardStats; initialActivities: ActivityItem[]; userEmail: string | null; shopName?: string; liveMode: boolean; canManageMembers?: boolean; initialError?: string; initialView?: DashboardView; initialCustomerId?: string }) {
  ...
  const [theme, setTheme] = useState<"light" | "dark">(() => {
    ...
    try {
      const saved = localStorage.getItem("creditbook-theme") as "light" | "dark" | null;
  ...
  function toggleTheme() {
    ...
    setTheme(next);
    try {
      localStorage.setItem("creditbook-theme", next);
    } catch {
      ...
      <main className="main-area">
        <header className="topbar">
          ...
          <div className="user-chip">
            <span>{userEmail ?? "Sinov rejimi"}</span>
            <div className="avatar">{userEmail ? initials(userEmail) : "SR"}</div>
            <button className="theme-toggle-btn" onClick={toggleTheme} aria-label="Rejimni almashtirish" title="Rejimni almashtirish">{theme === "dark" ? "☀️" : "🌙"}</button>
            <button className="icon-button mobile-notice" onClick={() => { setMoreOpen(false); navigateToView("reminders"); }} aria-label="Eslatmalarni ochish"><Bell size={19} /></button>
            <button className="icon-button" onClick={() => void handleLogout()} disabled={signingOut} aria-label="Chiqish" title="Chiqish"><LogOut size={18} /></button>
          </div>
        </header>
