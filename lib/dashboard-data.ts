...
export async function getDashboardData() {
  ...
  if (!supabase) return { initialCustomers: [], initialStats: emptyStats, initialActivities: [], userEmail: null, shopName: "Mahalla do'koni", liveMode: false, initialError: "Supabase ulanishi sozlanmagan." };
  ...
  return { initialCustomers: customers, initialStats, initialActivities: (activityData ?? []) as Array<{ id: string; customer_id: string | null; event_type: string; description: string; created_at: string }>, userEmail: userData.user.email ?? null, shopName, liveMode: true, initialError: "" };
}
